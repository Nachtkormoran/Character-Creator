...
export function CharacterForm({
  ...
  /**
   * Schon per KI vorgeschlagene Namen – gehen als Ausschlussliste in den
   * nächsten Vorschlag, damit wiederholtes Klicken nicht denselben Prior-Kopf
   * liefert (s. `buildNamePrompt`).
   */
  ...
  async function suggestName() {
    ...
    try {
      const { name } = await generateName(
        form,
        undefined,
        // Der aktuelle Feldwert zählt mit: auch ein gewürfelter oder getippter
        // Name soll nicht gleich wieder vorgeschlagen werden.
        Array.from(new Set([form.name, ...nameHistory].filter(Boolean))),
      );
      update("name", name);
